...
const STAGE_DETAILS: Record<string, string> = {
  ...
  rendering_pdf: "正在生成 Typst 中间稿并调用 Typst 编译 PDF。",
...
export function ProgressWindow() {
  ...
  const resolvedTranslatedTyp = task.translatedTyp ?? joinOutputPath(task.outputDir, "translated.typ");
  ...
  const translationEnabled = Boolean(task.translatedMd || task.translatedTyp || task.translatedPdf);

  const stepIndex =
    task.stage === "failed" || task.stage === "cancelled"
      ? 0
      : Math.max(["queued", "extracting", "preprocessing", "translating", "rebuilding", "rendering_pdf", "completed"].indexOf(task.stage), 0);
  ...
  async function handleRerenderPdf() {
    ...
    try {
      ...
      notifications.show({
        color: "appleBlue",
        title: "已开始重新生成 PDF",
        message: "这次不会重跑 MinerU 和翻译，只会重新生成 Typst 并编译 PDF。",
      });
    } catch (error) {
      notifications.show({
        color: "dark",
      ...
      <AppShell.Main className="settings-main">
        <Stack gap="lg">
          ...
          <Stepper active={stepIndex} orientation="vertical" color="appleBlue" iconSize={26}>
            ...
            <Stepper.Step
              label="回填与导出"
              description={translationEnabled ? "恢复结构并生成 Markdown、Typst、PDF 与报告" : "直接导出 raw.md 与 MinerU 资源"}
            />
          </Stepper>
          ...
          <Stack gap="sm">
            ...
            <Text size="sm" c="dimmed">
              完成后这里会显示导出的 Markdown、Typst、PDF 和报告位置。
            </Text>
            ...
            <Text size="sm" className="result-path">
              Typst: {resolvedTranslatedTyp ?? "等待生成"}
            </Text>
            <Text size="sm" className="result-path">
