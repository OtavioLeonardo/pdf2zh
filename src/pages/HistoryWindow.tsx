  ...
  async function handleRerenderPdf(entry: HistoryEntry) {
    if (!entry.translatedMd) {
      notifications.show({
        color: "gray",
        title: "暂时无法重新生成",
        message: "这个记录里还没有 translated.md，暂时不能直接重新生成 PDF。",
      });
      return;
    }
    ...
    try {
      ...
      notifications.show({
        color: "appleBlue",
        title: "已开始重新生成 PDF",
        message: "这次不会重新翻译，只会重新生成 Typst 并编译 PDF。",
      });
    } catch (error) {
      notifications.show({
        color: "dark",
            ...
            <Group gap="sm">
              ...
              <Box>
                ...
                <Text c="dimmed" size="sm">
                  统一展示 Documents/pdf2zh 目录下的翻译记录，方便重新打开 Typst、PDF 和输出目录。
                </Text>
              </Box>
            </Group>
          </Box>
                  ...
                  <Stack gap="sm">
                    <Group justify="space-between" align="flex-start">
                      ...
                      <Text size="sm" c={entry.pdfGenerated ? "appleBlue" : "dimmed"}>
                        {entry.pdfGenerated ? "PDF 已生成" : entry.translatedTyp ? "Typst 已生成" : entry.translatedMd ? "仅 Markdown" : entry.rawMd ? "仅 raw.md" : "处理中"}
                      </Text>
                    </Group>
